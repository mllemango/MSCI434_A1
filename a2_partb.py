...
def optimize(plants, WHs, custs, plant_cap, plant_cost, WH_cap, WH_Cost, cust_demand, plant_WH_distance, WH_cust_distance):
    ...
    Cjk = m.addVars(WHs, custs, vtype=GRB.INTEGER)  # number of goods from WH j to customer k
    Wj = m.addVars(WHs, vtype=GRB.BINARY)  # 1 if warehouse j operates
    Wjk = m.addVars(WHs, custs, vtype=GRB.BINARY)  # 1 if warehouse j services customer K
    ...
    for k in custs:
        ...
        m.addConstr(quicksum(Wjk[j, k] for j in WHs) == 1)  # only 1 warehouse to each customer

    for j in WHs:
        for k in custs:
            M = 10000000
            m.addConstr(M * Wjk[j, k] >= Cjk[j, k])  # cant transport goods in warehouse does not service customer
    ...
    print(solution)
    solution2 = m.getAttr('x', Wjk)
    for sol in solution2:
        print(sol, solution2[sol])
